...
func _ready() -> void:
	if !_billboard_videolist_initialization && !billboard_video_pool.is_empty():
		...
		_video_player=%VideoStreamPlayer as VideoStreamPlayer #NOTE: %VideoStreamPlayer refers to the Node of the same name (and type) but "Access as Unique Name" is set to true. it's a good habit to set nodes like this (which is gonna have their name used alot in this program for example) to a variable of the same type as the node.(for example, you change the name of the node and come back to script, you'll have to change just one word in the script which is the line this comment is on (the name after %)) oh and for more runtime safety use "as"+node_type(here it's "as VideoStreamPlayer") after the % decleration.
		_video_player.autoplay=true
		_video_player.volume_db=-80
		_video_player.finished.connect(_billboard_video_finished)
#-----Repeatable Tasks-----#
func _update_billboard_videopool_size(): #NOTE: instead of writing the whole update code everywhere, imma func it. (real usage: dynamic array size while in runtime (removal of array cells (example: my tv system's game jam version ensures video uniqueness per tape by removing the last video selected)))
	_shared_billboard_videopool_size=_shared_billboard_videopool.size()
func _billboard_video_randomizer(): #NOTE: in my origial code used in the game jam, i used whatever i found online. as i've tested just now, you do not need to use the "randomize()" function to get a new number, you just call this function.
	_update_billboard_videopool_size()
	_billboard_videopool_pointer=randi()%_shared_billboard_videopool_size
#-Cycler-#
func _billboard_video_finished():
	_billboard_video_randomizer()
